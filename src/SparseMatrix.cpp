// ═══════════════════════════════════════════════════════════════════
//  ARCHIVO : SparseMatrix.cpp
//  CARPETA : src/
//  AUTOR   : Miembro 1
//  ───────────────────────────────────────────────────────────────────
//  QUÉ HACE:
//    Implementa toda la lógica de la lista enlazada cruzada.
//    Es el archivo más importante del proyecto — aquí vive la
//    gestión manual de punteros que el enunciado evalúa con 4 pts.
//
//  QUÉ IMPLEMENTA:
//    · Constructor / Destructor    — crea y libera toda la memoria
//    · getOrCreateRow/Col          — obtiene o crea cabeceras
//    · insert                      — enlaza el nodo nuevo en fila
//                                    Y columna simultáneamente
//    · query                       — recorre la fila hasta la columna
//    · modify                      — edita value sin mover el nodo
//    · deleteCell                  — ★ punto crítico: remenda los dos
//                                    punteros (fila y columna) antes
//                                    de hacer delete
//    · deleteRow                   — elimina todos los nodos de la fila
//                                    reparando cada columna afectada
//    · deleteCol                   — elimina todos los nodos de la col
//                                    reparando cada fila afectada
//    · deleteRange                 — llama deleteCell por cada posición
//    · parseCoord / coordToString  — conversión "B3" ↔ {2,1}
//    · printAll                    — debug: imprime toda la hoja
//
//  ADVERTENCIA:
//    Las funciones deleteCell, deleteRow y deleteCol son las más
//    propensas a errores. Si los punteros no se remendan en AMBAS
//    direcciones (fila Y columna) antes del delete, quedan punteros
//    colgantes que causan comportamiento indefinido.
// ═══════════════════════════════════════════════════════════════════
...
RowHeader* SparseMatrix::getOrCreateRow(int row) {
    ...
    while (cur && cur->row < row) { prev = cur; cur = cur->nextRow; }
    if (cur && cur->row == row) return cur;

    RowHeader* nh = new RowHeader(row);
    nh->nextRow = cur;
    if (prev) prev->nextRow = nh;
    else       rowList_     = nh;
    return nh;
}
...
ColHeader* SparseMatrix::getOrCreateCol(int col) {
    ...
    while (cur && cur->col < col) { prev = cur; cur = cur->nextCol; }
    if (cur && cur->col == col) return cur;

    ColHeader* nh = new ColHeader(col);
    nh->nextCol = cur;
    if (prev) prev->nextCol = nh;
    else       colList_     = nh;
    return nh;
}
...
RowHeader* SparseMatrix::getRowHead(int row) const {
    RowHeader* rh = rowList_;
    while (rh && rh->row != row) rh = rh->nextRow;
    return rh;
}

ColHeader* SparseMatrix::getColHead(int col) const {
    ColHeader* ch = colList_;
    while (ch && ch->col != col) ch = ch->nextCol;
    return ch;
}
...
void SparseMatrix::removeRowHeaderIfEmpty(int row) {
    ...
    RowHeader* cur  = rowList_;
    while (cur && cur->row != row) { prev = cur; cur = cur->nextRow; }
    if (!cur || cur->first) return;   // no existe o aún tiene nodos
    if (prev) prev->nextRow = cur->nextRow;
    else       rowList_     = cur->nextRow;
    delete cur;
}
...
void SparseMatrix::removeColHeaderIfEmpty(int col) {
    ...
    ColHeader* cur  = colList_;
    while (cur && cur->col != col) { prev = cur; cur = cur->nextCol; }
    if (!cur || cur->first) return;
    if (prev) prev->nextCol = cur->nextCol;
    else       colList_     = cur->nextCol;
    delete cur;
}
...
void SparseMatrix::insert(int row, int col, CellValue value) {
    ...
    Node* prevR = nullptr;
    Node* cur   = rh->first;
    while (cur && cur->col < col) { prevR = cur; cur = cur->nextInRow; }
...
std::optional<CellValue> SparseMatrix::query(int row, int col) const {
    ...
    Node* n = rh->first;
    while (n && n->col < col) n = n->nextInRow;
    if (n && n->col == col) return n->value;
    return std::nullopt;
}
...
void SparseMatrix::deleteCell(int row, int col) {
    RowHeader* rh = getRowHead(row);
    if (!rh) return;   // fila vacía — sin crash

    // Remendar enlace en fila
    Node* prevR = nullptr;
    Node* cur   = rh->first;
    while (cur && cur->col < col) { prevR = cur; cur = cur->nextInRow; }
    if (!cur || cur->col != col) return;   // celda no existe — sin crash

    if (prevR) prevR->nextInRow = cur->nextInRow;
    ...
    Node* prevC   = nullptr;
    Node* curC    = ch->first;
    while (curC && curC != cur) { prevC = curC; curC = curC->nextInCol; }

    if (prevC) prevC->nextInCol = cur->nextInCol;
    else        ch->first       = cur->nextInCol;
...
void SparseMatrix::deleteRow(int row) {
    ...
    while (rh2 && rh2->row != row) { prev = rh2; rh2 = rh2->nextRow; }
    if (prev) prev->nextRow = rh2->nextRow;
    else       rowList_     = rh2->nextRow;
    delete rh2;
}
...
void SparseMatrix::deleteCol(int col) {
    ...
    while (cur) {
        ...
        Node* prevR   = nullptr;
        Node* curR    = rh->first;
        while (curR && curR != cur) { prevR = curR; curR = curR->nextInRow; }
        if (prevR) prevR->nextInRow = cur->nextInRow;
        else        rh->first       = cur->nextInRow;
    ...
    while (ch2 && ch2->col != col) { prev = ch2; ch2 = ch2->nextCol; }
    if (prev) prev->nextCol = ch2->nextCol;
    else       colList_     = ch2->nextCol;
    delete ch2;
}
...
void SparseMatrix::printAll() const {
    ...
    while (rh) {
        for (Node* n = rh->first; n; n = n->nextInRow) {
            ...
            std::visit([](auto&& v){ std::cout << v; }, n->value);
            std::cout << "  ";
        }
        std::cout << "\n";
        rh = rh->nextRow;
